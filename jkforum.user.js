...
function getData(url, type = "document" /* DOCUMENT */, usermethod = "GET" /* GET */) {
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            ontimeout: () => {
                new _1.MessageBox('网络超时', 5000, "LOG_POP_GM" /* LOG_POP_GM */);
                reject('timeout');
            },
        });
    });
            ...
            image.onload = function () {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (typeof code === 'string') {
                        ...
                        if (code.error_msg === exports.RETRY) {
                            new _1.MessageBox('识别失败，正在重试...');
                            return reject(exports.RETRY);
                        }
                        else if (code.type === 'token_error') {
                            new _1.MessageBox('令牌错误，请检查设置：' + code.error_msg, 'none', "LOG_POP_GM" /* LOG_POP_GM */);
                            return reject(code);
                        }
                        else if (code.type === 'server_error') {
                            new _1.MessageBox(`服务器内部错误，将在 ${thread.cycle} 分钟后自动重试，多次重试未恢复请联系管理员`, 60000 * Number(thread.cycle), "LOG_POP_GM" /* LOG_POP_GM */);
                            return resolve(code.error_msg);
                        }
                        else {
                            // 其他错误情况
                            new _1.MessageBox('运行错误，正在重试：' + code.error_msg);
                            return reject(exports.RETRY);
                        }
                    }
                });
...
function readImage(base64, user) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const response = yield (0, _1.postData)(user.ocrUrl, body, {
            ...
            var _a;
            if (e === 'timeout' || ((_a = e === null || e === void 0 ? void 0 : e.message) === null || _a === void 0 ? void 0 : _a.includes('network'))) {
                return { error_msg: exports.RETRY };
            }
            // 其他未知错误也返回为重试，解析错误消息
            let msg;
            if (e.response) {
        ...
        try {
            ...
            if (t.runTime) {
                ...
                if (hours !== 0) {
                    ...
                    const overMinutes = now.getMinutes() * 60000 + now.getSeconds() * 1000 - (0, tools_1.rdNum)(0, 120000);
...
const AutoClickManage = ({ onClose, user }) => {
    ...
    const triggerNextClick = (t) => {
        ...
        if (diff < 600000 && t.retry < 10) {
            ...
            // 添加任务，在此处开始闭包，递归调用
            pool.all([() => (0, lib_1.autofillCaptcha)(onThread, user, setNextClickTime, saveStatusData, triggerNextClick)]);
            new lib_1.MessageBox(`帖子ID：${(0, tools_1.getTid)(onThread.url)}，已错过点击时间，重置点击时间到下一个点击周期！预设点击时间：${new Date(t.nextClickTime).toLocaleString()}，实际时间：${new Date().toLocaleString()}`);
        }
    };
...
const components_1 = __webpack_require__(740);
const lib_1 = __webpack_require__(915);
const autoPost_1 = __webpack_require__(342);
