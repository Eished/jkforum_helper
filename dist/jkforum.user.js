...
const AutoClickManage = ({ onClose, user }) => {
    ...
    const setNextClickTime = (t) => {
        setData((old) => old.map((row) => {
            if (row.url === t.url) {
                return Object.assign(Object.assign({}, row), { times: row.times + 1, nextClickTime: t.nextClickTime });
            }
            return row;
        }));
    };
